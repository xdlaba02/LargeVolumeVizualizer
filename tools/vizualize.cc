...
void parse_args(int argc, const char *argv[], const char *&processed_volume, const char *&processed_metadata, const char *&transfer_function, uint32_t &width, uint32_t &height, uint32_t &depth) {
  if (argc != 7) {
...
int main(int argc, const char *argv[]) {
  try {
    ...
    parse_args(argc, argv, processed_volume, processed_metadata, transfer_function, width, height, depth);
    ...
    std::cerr << e.what() << "\n";
    std::cerr << "Usage: \n";
    std::cerr << argv[0] << " <processed-volume> <processed-metadata> <transfer-function> <width> <height> <depth>\n";
  }
