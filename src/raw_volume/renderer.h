...
glm::vec4 render(const RawVolume<T> &volume, const Ray &ray, float stepsize, const TransferFunctionType &transfer_function) {
  glm::vec4 dst(0.f, 0.f, 0.f, 1.f);

  RayRange range;

  intersect_aabb_ray(ray.origin, ray.direction_inverse, {0, 0, 0}, { 1, 1, 1 }, range.min, range.max);
